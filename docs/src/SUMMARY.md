# Summary

[Introduction](introduction.md)
[Quickstart](quickstart.md)

# Using the fork

- [Supported versions](guide/versions.md)
- [Installation](guide/installation.md)
- [Running broadwayd](guide/running.md)
- [Deploying behind TLS](guide/deployment.md)
- [Configuration reference](guide/config.md)
- [Known issues & tested configs](guide/known-issues.md)

# Features

- [Backend comparison](features/comparison.md)
- [Clipboard](features/clipboard.md)
  - [Implementation](internals/clipboard.md)
- [Touch interface](features/touch.md)
  - [Implementation](internals/touch.md)
- [Pinch to zoom](features/zoom.md)
  - [Implementation](internals/zoom.md)
- [Notebook tabs](features/notebook.md)
  - [Implementation](internals/notebook.md)
- [Opening links](features/open-uri.md)
  - [Implementation](internals/open-uri.md)
- [Connection management](features/connection.md)
  - [Implementation](internals/connection.md)
- [Scaling & HiDPI](features/scaling.md)
- [Desktop & rendering fixes](features/desktop-fixes.md)

# Internals

- [Architecture](internals/architecture.md)
  - [broadwayd vs libgtk](internals/build-split.md)
- [Wire protocol](internals/protocol.md)
- [Input region & pointer](internals/input-region.md)
- [Rendering & performance](internals/performance.md)
- [Debug menu](internals/debug-menu.md)

# Building & releasing

- [Build from source](build/from-source.md)
- [Re-forking a new GTK release](build/reforking.md)
- [CI & packaging](build/ci.md)
- [Release process](build/release.md)

---

[Changelog](changelog.md)

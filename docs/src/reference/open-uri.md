...
X11/Wayland get their own GIRs, Broadway gets none), so Python can't call a new Broadway GDK
function directly. So a new public `gdk_broadway_display_show_uri()` (`gdkdisplay-broadway.c`) is
reached through an **introspectable GTK choke point**: `gtk_show_uri_full`

...
The fork ships as a Debian package, `gtk4-broadway-fork`, built by CI and published to [GitHub Releases](https://github.com/droserasprout/gtk-broadway/releases).
...
The asset name encodes base and architecture: `gtk4-broadway-fork_<gtk>-<rev>_<arch>.deb` (e.g. `gtk4-broadway-fork_4.22.2-2.1_amd64.deb`).

> To always grab the newest release without hardcoding the tag, resolve it from the GitHub API first: ```sh rel="$(curl -fsSL https://api.github.com/repos/droserasprout/gtk-broadway/releases/latest \ | grep -oP '"tag_name":\s*"\K[^"]+')" ```

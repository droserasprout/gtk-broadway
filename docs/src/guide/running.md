...
The clipboard bridge uses `navigator.clipboard`, which browsers only expose in a secure context: `https://` or `http://localhost`. Over plain `http://` to a remote host, copy may silently fail outside a user gesture. The reference deployment runs behind TLS (Traefik in Docker Swarm) for exactly this reason.

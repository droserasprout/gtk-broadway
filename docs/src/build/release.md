...
A release is one immutable GitHub Release holding every base x arch `.deb`. You cut it by
pushing a `vN` tag to the `ci` branch, which triggers `release.yml`.

## Tagging a release

Each base builds from a pinned per-version tag `<gtk>-N`. Create those first, then push the
orchestrator `vN` tag to trigger the build. For `v123`:
...
The revision `N` is a single rolling number shared across both bases for a given release, and it

...
Triple-Shift in `broadway.js` sends `BROADWAY_EVENT_MENU` (17). `broadwayd` intercepts it and `spawn`s `gtk4-brotway-debugmenu` with `GDK_BACKEND`/`BROADWAY_DISPLAY` pointed at itself, so the window renders into the same display and is pinned always-on-top via server-side stacking. `EVENT_MENU` comes from the untrusted browser, so a failed spawn (binary missing) backs off 10s before retrying. The summoned menu is matched by client id: only a client that connected *after* the summon can become the menu owner, and the expectation expires after 10s - so a dialog or drag surface another app maps during the spawn window can't get pinned on top. The control sources and child watch are torn down when the server finalizes (they hold the server raw).

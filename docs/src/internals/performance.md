...
The profiler lives in the [debug menu](debug-menu.md); see there for how it is wired into `broadwayd`. This section covers reading it.
...
An earlier fast preset used a single Sub filter + `Z_RLE` for cheaper encode, but it compresses anti-aliased text and gradients far worse than adaptive filtering and grew the output ~50% median, shifting cost to client transfer and decode - what scrolling is bound by. The filter, not the level, drove the bloat: level 3 with adaptive filtering encodes cheaper while staying within ~5-12% of the level-6 default, so fast keeps adaptive filtering at the low level.
...
Deduping repeated **show-keyboard** / **input-region** ops the same way was reverted: broadwayd never resets that state on client disconnect, so the redundant re-sends double as a reconnect resync. Suppressing them left a stale OSK or empty-input-region state on mobile. Safe only once disconnect resets the flags or the reconnect sync re-asserts both states.
...
**Notebook edge-fade (solved natively).** The [notebook](../features/widgets.md#notebook-tabs) tab-scroll edge-fade used `gtk_snapshot_push_mask`, which Broadway rasterizes (no `GskMaskNode` renderer) into a texture re-uploaded every scroll frame. It is now drawn as themed CSS `undershoot` nodes: native `GSK_LINEAR_GRADIENT_NODE`s fading into the header `$dark_fill`. Zero texture traffic, theme-correct.

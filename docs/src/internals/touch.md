...
The user-facing summary is in [Touch interface](../features/touch.md). This page covers the
individual fixes. Each is tagged **broadwayd-only** (`broadway.js` / `client.html` / daemon C) or
**libgtk** (GDK / GTK widgets); see [broadwayd vs libgtk](build-split.md).
...
re-assertion through the browser, covered in [Input region & pointer](input-region.md).

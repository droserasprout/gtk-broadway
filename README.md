...
A fork of GTK **4.14.5** adding missing features to the **Broadway** backend - GTK's HTML5 renderer that serves an app to web browser over a WebSocket.
...
A Debian package `gtk4-broadway-fork` built by CI on `ubuntu:24.04` and published to the rolling `4.14.5-latest` GitHub Release. It Depends/Replaces `libgtk-4-1` and `libgtk-4-bin`, overlaying only the patched `libgtk-4.so` and `gtk4-broadwayd` keeping the rest of GTK in place:

```sh
wget -O gtk.deb "https://github.com/droserasprout/gtk-broadway/releases/download/4.14.5-latest/libgtk4-broadway-fork_$(dpkg --print-architecture).deb"

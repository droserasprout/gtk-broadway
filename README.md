...
The patches are maintained per GTK version on dedicated branches; this `ci` branch holds only the docs and the build/release automation that compiles them into Debian packages.
...
| GTK    | Ubuntu base    | Fork branch   |
| ------ | -------------- | ------------- |
| 4.14.5 | `ubuntu:24.04` | `4.14.5-fork` |
| 4.22.2 | `ubuntu:26.04` | `4.22.2-fork` |

## Installation

A Debian package `gtk4-broadway-fork` is built by CI and published to an immutable `vN` GitHub Release. It Depends/Replaces `libgtk-4-1` and `libgtk-4-bin`, overlaying only the patched `libgtk-4.so` and `gtk4-broadwayd` and keeping the rest of GTK in place.

Pick the asset matching your Ubuntu base (`24.04` -> `4.14.5`, `26.04` -> `4.22.2`) and architecture:
...
- `ubuntu:26.04` ships `4.22.2` - done ✅
